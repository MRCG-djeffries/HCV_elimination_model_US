fit_to_data_module_ui <- function(id) {
  ...
  dashboardPage(
    ...
    dashboardSidebar(width = 320, shinyjs::useShinyjs(),
                     ...
                     div(style="display: inline-block;vertical-align:top;",actionBttn(ns("model_fit1"), label="Fit PWID model",size="xs")),
                     #div(style="display: inline-block;vertical-align:top;",actionBttn(ns("model_fit2"), label="Fit PWID model 2",size="xs")),
                     div(style="display: inline-block;vertical-align:top;",actionBttn(ns("genpop_fit_butt"), label="Fit Gen Pop",size="xs")),
                     tags$hr(),
...
fit_to_data_module <- function(input, output, session) {
  ...
  observeEvent(input$model_fit1, {
    ...
    output$plot_model =  renderPlot(grid.arrange(X$p1,X$p2,X$p3,X$p4,X$p5,nrow = 2,heights = c(1, 1),layout_matrix=rbind(c(1,1,2),c(3,4,5))))
    ns =session$ns
    output$ui =renderUI(   plotOutput(ns("plot_model"))  )
  })
  observeEvent(input$reset_input,{
    output$plot_model =  renderPlot(NULL)
    output$plot_text = renderUI({HTML("Fitting epidemic data to the PWID and general population models for the US")})
  })
  ...
  output$plot_text = renderUI({HTML("The model is fit to the epideic data, by varying calibration parameters")})
